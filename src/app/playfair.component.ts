...
export class PlayfairComponent implements OnInit {
  ...
  cipher = '';
  plain = '';
  printkey = 'a b c d e<br/>f g h i k<br/>l m n o p<br/>q r s t u<br/>v w x y z';

  /*! fairplay - v0.1.0 - 2014-03-02
  ...
  fairplay = {
    ...
    generateKey: function (phrase) {
      let alphabet: any = (function () {
          var alph = {},
            i = 0,
            l = 0;

          for (let i = 97, l = 105; i <= l; i += 1) {
            alph[String.fromCharCode(i)] = false;
          }

          for (let i = 107, l = 122; i <= l; i += 1) {
            alph[String.fromCharCode(i)] = false;
          }
      ...
      for (let i = 0, l = phrase.length; i < l; i += 1) {
  ...
  enkrip(key, plaintext, pc) {
    this.fairplay.monograph = pc;
    this.printkey = this.fairplay.printKey(key, "<br />");

    this.cipher = this.fairplay.encrypt(key, plaintext);
    console.log(this.cipher);
    console.log(this.printkey);
  }

  dekrip(key, cipher) {
    this.printkey = this.fairplay.printKey(key, "<br />");

    this.plain = this.fairplay.decrypt(key, cipher);
